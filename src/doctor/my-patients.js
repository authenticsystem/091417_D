...
class MyPatients extends PolymerElement {
        ...
        <iron-pages attr-for-selected="name" selected="[[route.path]]" fallback-selection="view404">
            ...
            <div name="/new/p">
                <template is="dom-if" if="[[newPActive]]" restamp>
                    <patient-form prev-route="[[route.prefix]]" params="[[route.__queryParams]]" header="New Patient" my-data="[[myData]]" my-code="[[myData.code]]"></patient-form>
                </template>
            </div>
            <div name="/edit/p/">
                <template is="dom-if" if="[[editPActive]]" restamp>
                    <patient-form prev-route="[[route.prefix]]" params="[[route.__queryParams]]" header="[[selectedPatient.no]]" my-data="[[myData]]" my-code="[[myData.code]]"></patient-form>
                </template>
            </div>
            <div name="/detail/p/">
                <template is="dom-if" if="[[detailPActive]]" restamp>
                    <patient-view prev-route="[[route.prefix]]" params="[[route.__queryParams]]" header="[[selectedPatient.no]]" my-data="[[myData]]" my-code="[[myData.code]]"></patient-view>
                </template>
            </div>
            <div name="view404">
    ...
    _query(value, orderChild, callback) {
        this._debounceJob = Debouncer.debounce(this._debounceJob,
            timeOut.after(500), () => {
                firebase.database().ref('/' + this.myData.code + '/patients').orderByChild(orderChild).startAt(value).limitToFirst(15).once('value', function (snapshot) {
