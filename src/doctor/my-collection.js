...
class MyCollection extends PolymerElement {
        ...
        <iron-pages attr-for-selected="name" selected="[[route.path]]" fallback-selection="view404">
            ...
            <div name="/new/c">
                <template is="dom-if" if="[[newCActive]]" restamp>
                    <collection-form prev-route="[[route.prefix]]" my-data="[[myData]]" my-code="[[myData.code]]"></collection-form>
                </template>
            </div>
            <div name="/detail/c/">
                <template is="dom-if" if="[[detailCActive]]" restamp>
                    <collection-view prev-route="[[route.prefix]]" params="[[route.__queryParams]]" header="[[selectedCollection.ref]]" my-data="[[myData]]" my-code="[[myData.code]]"></collection-view>
                </template>
            </div>
            <div name="view404">
    ...
    static get MyList() {
        ...
        <app-header-layout fullbleed="" has-scrolling-region="">
            ...
            <iron-scroll-threshold id="threshold" on-lower-threshold="_loadMoreData">
                <iron-list items="[[collection]]" scroll-target="threshold">
                    <template>
                        <div>
                            <div class="item">
                                <item-card on-tap="_select" item="[[item]]" my-key="[[myData.code]]" query-to="collection_data"
    ...
    _query(value, orderChild, callback) {
        this._debounceJob = Debouncer.debounce(this._debounceJob,
            timeOut.after(500), () => {
                this.firebaseRef.child('/' + this.myData.code + '/collection').orderByChild(orderChild).startAt(value).limitToFirst(15).once('value', function (snapshot) {
    ...
    _drop(e) {
        ...
        if (confirm("Sure you want to remove collection #" + data.ref + "?\nThis will return billing data to not collected state and the amount will be added back to receivables.\nThere's no undo.")) {
            this.firebaseRef.child("/" + this.myData.code + "/collection_data").orderByChild("collection_ref").equalTo(data.ref).once('value', function (snapshot) {
                var i = 0, length = snapshot.numChildren();
                snapshot.forEach(element => {
                    this.firebaseRef.child("/" + this.myData.code + "/collection_data").child(element.key).remove();
                    this.firebaseRef.child("/" + this.myData.code + "/billing_data/" + element.val().billing_data_key).update({
                        patient_billTo_collected: element.val().patient + "_" + element.val().collectFrom + "_false",
                        collected: false,
                        collection_data_key: null,
                        collection_key: null
                    });
                    this.firebaseRef.child("/" + this.myData.code + "/receivables/" + element.val().service_collect_from).once('value', function (receivables) {
                        var total = receivables.val() ? receivables.val() : 0;
                        total += element.val().amount;
                        this.firebaseRef.child("/" + this.myData.code + "/receivables/" + element.val().service_collect_from).set(total);
                    }.bind(this));
                    i++;
                });
                if (i == length) {
                    this.firebaseRef.child("/" + this.myData.code + "/collection").child(data.$key).remove();
