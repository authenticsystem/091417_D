...
class MyMessaging extends PolymerElement {
        ...
        <app-drawer-layout fullbleed="" narrow="{{narrow}}" id="drawerLayout">
            <app-drawer id="drawer" slot="drawer" swipe-open="[[narrow]]" align="right">
                ...
                <paper-listbox class="mnu" selected="{{selectedContact}}" attr-for-selected="name">
                    <div hidden="[[isLoadingHidden]]" class="loading-indicator">
                        <paper-spinner active></paper-spinner>
                    </div>
                    <template is="dom-repeat" items="[[mySecretary]]">
                        <paper-icon-item style="text-align:left;" name="[[item.code]]~[[item.name.firstname]]">
                            <g-avatar label="[[item.name.firstname]]" slot="item-icon"></g-avatar>
    ...
    ready() {
        ...
        firebase.database().ref(`/${this.myData.code}/secretary`).on('value', function (snapshot) {
            this.isLoadingHidden = true;
            if (snapshot.exists()) {
                this.mySecretary = fbSnapshotToArray(snapshot);
                this.selectedContact = this.mySecretary[0].code + '~' + this.mySecretary[0].name.firstname;
            }
        }.bind(this));
    }
    ...
    _selectedContactChanged(contact) {
        ...
        this._debounceJob = Debouncer.debounce(this._debounceJob, timeOut.after(500), () => {
            ...
            firebase.database().ref(`/${selectedContactCode}/messages/${this.myData.code}`).limitToLast(15).on('value', function (snapshot) {
                if (snapshot.exists()) {
    ...
    _sendMsg() {
        if (this.msg && this.selectedContact) {
            var msg = this.msg;
            var push = firebase.database().ref("/" + this.selectedContactCode + "/messages/" + this.myData.code).push({
            ...
            if (push.key) {
                var ref = firebase.database().ref("/" + this.selectedContactCode + "/doctors");
    ...
    _msgReset(e) {
        var ref = firebase.database().ref("/" + this.myData.code + "/secretary");
