...
import './billing-datatable.js';

class BillingForm extends PolymerElement {
    constructor() {
        super();
            ...
            <app-header-layout has-scrolling-region="">
                <app-header fixed="" slot="header">
                    <app-toolbar>
                        ...
                        <div main-title="">New Billing</div>
                    </app-toolbar>
                </app-header>
                ...
                <neon-animated-pages selected="[[selectedPage]]" entry-animation="[[entryanimation]]" exit-animation="[[exitanimation]]">
                    ...
                    <neon-animatable>
                        <billing-datatable my-data="[[myData]]" my-code="[[myCode]]" patient="[[selectedPatient.personal.name]]" patient-hmo="[[selectedPatient.insurance.primary_insurance]]" data="{{billingData}}" total="{{billingTotal}}" remarks="{{billingRemarks}}" date-issued="{{dateIssued}}" on-page-changed="_transactionPageChanged"></billing-datatable>
                    </neon-animatable>
                        ...
                        <div class="polymer-card">
                            <div>
                                <div id="saveSuccess" style="color: green; display:none">Billing successfully saved.
                                    <p style="margin-top:-4px">Billing ref.# <b style="color:#1976d2">[[reference]]</b></p>
                                    <a href="[[prevRoute]]">
    ...
    static get observers() {
        return [
            ...
            '_showSaved(billingData.length, dateIssued)'
        ];
    }
    ...
    _selectedPageChanged(page) {
        switch (page) {
            ...
            case 1:
                ...
                if (this.billingData.length > 0 && this.dateIssued) this.favrighthide = false;
    ...
    async _save() {
        var newBillingRef, updateAtOnce = {}, num = 0;
        var billing = {
        ...
        if (this.dateIssued < formatDate(new Date(), 'yyyy-mm-dd')) {
            newBillingRef = await generatePushID(this.dateIssued, this.myCode);
            billing.timestamp = newBillingRef.timestamp;
            billing.order = newBillingRef.timestamp;
        }
        else {
            newBillingRef = await this.firebaseRef.child("/" + this.myCode + "/billing").push();
            billing.timestamp = firebase.database.ServerValue.TIMESTAMP;
            billing.order = firebase.database.ServerValue.TIMESTAMP;
        }

        const receivablesRef = "/" + this.myCode + "/receivables";
        const numRef = "/" + this.myCode + "/uniqueId/billing";
        const billingRef = "/" + this.myCode + "/billing/" + newBillingRef.key;
        ...
        this.firebaseRef.child(billingRef).once('value', function (snapshot) {
            const order = snapshot.val().order * -1;
            this.firebaseRef.child(billingRef).update({ order: order });
        }.bind(this));

        this.billingData.forEach(element => {
            element.billing_key = newBillingRef.key;
...
window.customElements.define('billing-form', BillingForm);
